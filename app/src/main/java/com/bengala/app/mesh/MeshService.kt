...
import com.bengala.app.MainActivity
import com.bengala.app.R
import com.bengala.app.crypto.CryptoBox
import com.bengala.app.data.ChatMessage
import com.bengala.app.data.MeshRepository
import com.bengala.app.data.PeerLocation
import com.bengala.app.data.Settings
import com.bengala.app.location.LocationEngine
...
class MeshService : Service() {
        ...
        scope.launch {
            while (true) {
                ...
                try {
                    ...
                    android.util.Log.w("Bengala", "Watchdog", e)
                }
            }
        }
    }
                ...
                if (json.optInt("rm", 0) == 1) {
                    ...
                    if (!la.isNaN() && !lo.isNaN()) {
                        MeshRepository.setMeetPoint(
                            com.bengala.app.data.MeetPoint(la, lo, name, packet.timestamp),
                        )
                    }
                }
            }
    ...
    fun sendMeetPoint(latitude: Double, longitude: Double, remove: Boolean = false) {
        ...
        if (!remove) {
            MeshRepository.setMeetPoint(
                com.bengala.app.data.MeetPoint(
    ...
    fun onSettingsChanged() {
        ...
        scope.launch {
            try {
                ...
                android.util.Log.w("Bengala", "No se pudo aplicar ajustes", e)
            }
        }
    }
    ...
    private fun calibrateRssi() {
        ...
        for (peer in MeshRepository.peers.value.values) {
            ...
            val d = com.bengala.app.location.Geo.distanceMeters(
    ...
    private fun buildNotification(): Notification {
        val channelId = "bengala_mesh"
        if (Build.VERSION.SDK_INT >= 26) {
            val nm = getSystemService(NotificationManager::class.java)
            nm.createNotificationChannel(
                NotificationChannel(
                    channelId, "Mesh Bengala", NotificationManager.IMPORTANCE_LOW,
                ),
            )
        }
