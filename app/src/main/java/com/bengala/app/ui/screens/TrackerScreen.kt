 ...
 */
@Composable
fun TrackerScreen(target: com.bengala.app.data.TrackTarget) {
    ...
    val isMeet = target is com.bengala.app.data.TrackTarget.MeetTarget
    val peerId = (target as? com.bengala.app.data.TrackTarget.PeerTarget)?.id
    val peer = peerId?.let { peers[it] }
    val targetName = if (isMeet) "EL PUNTO ⚑" else (peer?.name ?: "???").uppercase()
    val loc = if (isMeet) {
        meet?.let {
            com.bengala.app.data.PeerLocation(it.latitude, it.longitude, 5f, it.timestamp)
        }
    } else peer?.location
    val me = myLocation
