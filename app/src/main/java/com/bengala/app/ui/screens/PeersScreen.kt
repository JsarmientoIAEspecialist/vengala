...
import com.bengala.app.data.Peer
import com.bengala.app.data.TrackTarget
...
fun PeersScreen() {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        if (peers.isEmpty()) {
            Text(
                "Todavía no aparece nadie.\n\nDile a tu parche que abra Bengala con el " +
...
private fun DiagnosticsCard(
    ...
    Column(
        ...
        DiagRow(
            "Teléfonos Bengala detectados", "$devicesFound",
            devicesFound > 0,
        )
        ...
        if (devicesFound == 0 && advertise == "ok" && scan == "ok") {
            Text(
                "Escaneando bien pero sin detectar a nadie: verifica que el otro teléfono " +
                    "tenga Bengala ABIERTA, Bluetooth encendido y su diagnóstico en verde.",
                style = MaterialTheme.typography.labelSmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
        }
