...
/**
 * Comparte el APK instalado de Bengala por Bluetooth (o Quick Share, etc.).
...
object ApkSharer {
    ...
    fun shareApk(context: Context) {
        try {
            ...
            val outFile = File(outDir, "bengala.apk")
            sourceApk.copyTo(outFile, overwrite = true)

            val uri = FileProvider.getUriForFile(
                context, "com.bengala.app.fileprovider", outFile,
            )
            ...
            context.startActivity(
                Intent.createChooser(intent, "Compartir Bengala por Bluetooth"),
            )
        } catch (e: Exception) {
